...
\begin{frame}{Network learned from the \textit{S. cerevisiae} data}
  ...
  \begin{tikzpicture}
   ...
   \uncover<2>{
    \fill[fill=white,opacity=0.7,even odd rule]
...
\centering
\begin{tikzpicture}
 \node[anchor=north west,inner sep=0,clip] at (-8cm,5cm) {\includegraphics[width=\textwidth]{go-terms-dense-csnem-presentation}};
 \path pic {colormap};
 \draw[help lines] (-5cm,-5cm) grid (5cm,5cm);
\end{tikzpicture}
\end{frame}
...
\begin{frame}{Conclusions}
 \begin{itemize}
  \item We introduce a novel type of model context-specific biological interactions.
  \pause
