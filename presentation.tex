...
\begin{tikzpicture}[very thick, ampersand replacement=\&]
 ...
 \uncover<1>{
  \node[text block,anchor=south,text width=8cm] at (cell.south) {\textbf{Task:} infer {\color{StrongBlue} causal relationships} from \color{ActionRed}{observed high-dimesional phenotypes (\textbf{effects})}};
 }
 \uncover<2>{
...
\begin{frame}{Evaluation on simulated data}
 \begin{tikzpicture}
  ...
  \node[text block] (tf) at (80mm, 0) {$F_\text{true}$};
  ...
  \pause
  \node[text block] (lf) at (110mm,-40mm) {$F_\text{learned}$};
...
\begin{frame}{Network learned from the \textit{S. cerevisiae} data}
  ...
  \begin{tikzpicture}
   ...
   \uncover<2>{
    \fill[fill=white,opacity=0.85,even odd rule]
      (-1cm,1cm) rectangle (13cm,-5cm)
      (38mm,4mm) circle (5mm);
    \node[text block, text width=8cm] at (6cm,-3cm) {Hog1 is a master regulator of osmotic stress response};  
   }
   
   \uncover<3>{
    \fill[fill=white,opacity=0.85,even odd rule]
      (-1cm,1cm) rectangle (13cm,-5cm)
      (38mm,4mm) circle[radius=5mm]
      (21mm,-4mm) circle[radius=5mm]
      (109mm,-26mm) circle[radius=5mm]
      (103mm,-41mm) circle[radius=5mm];
    \node[text block, text width=8cm] at (35mm,-4cm) {Hog1 and CK complex subunits (Cka2, Ckb1/2) interact, regulate overlapping sets of genes};  
   }
   ...
   \uncover<4>{
    \fill[fill=white,opacity=0.85,even odd rule]
      (-1cm,1cm) rectangle (13cm,-5cm)
      (38mm,4mm) circle[radius=5mm]
      (37mm,-5mm) circle[radius=5mm]
      (22mm,-15mm) circle[radius=5mm]
      (89mm,5mm) circle[radius=5mm]
      (109mm,4mm) circle[radius=5mm]
      (125mm,3mm) circle[radius=5mm]
      (125mm,-11mm) circle[radius=5mm];
    \node[text block, text width=8cm] at (60mm,-4cm) {Msn2 is known to be regulated by Hog1, Pde2, and Snf1, and Pde2 plays a more significant role in regulating Msn2 during salt stress};  
   }

   \uncover<5>{
    \fill[fill=white,opacity=0.85,even odd rule]
      (-1cm,1cm) rectangle (13cm,-5cm)
      (73mm,5mm) circle[radius=5mm] %ygr122w
